...
private[dynamodb]
class DynamoDBBackedBlockRDDPartition(
                                      idx: Int,
                                      blockId: BlockId,
...
/**
  ...
  */
private[dynamodb]
class DynamoDBBackedBlockRDD[T: ClassTag](
                                          sc: SparkContext,
                                          ...
                                          val messageHandler: Record => T = DynamoDBInputDStream.defaultMessageHandler _,
                                          val kinesisCreds: SparkAWSCredentials = DefaultCredentials,
                                          val kinesisReadConfigs: DynamoDBReadConfigurations = DynamoDBReadConfigurations()
  ...
  override def getPartitions: Array[Partition] = {
    Array.tabulate(_blockIds.length) { i =>
      ...
      new DynamoDBBackedBlockRDDPartition(i, _blockIds(i), isValid, arrayOfseqNumberRanges(i))
    }
  }

  override def compute(split: Partition, context: TaskContext): Iterator[T] = {
    val blockManager = SparkEnv.get.blockManager
    val partition = split.asInstanceOf[DynamoDBBackedBlockRDDPartition]
    ...
    def getBlockFromKinesis(): Iterator[T] = {
      ...
      partition.seqNumberRanges.ranges.iterator.flatMap { range =>
        new DynamoDBSequenceRangeIterator(credentials, endpointUrl, regionName,
...
/**
  ...
  */
private[dynamodb]
class DynamoDBSequenceRangeIterator(
                                    credentials: AWSCredentials,
                                    endpointUrl: String,
                                    regionId: String,
                                    range: SequenceNumberRange,
                                    kinesisReadConfigs: DynamoDBReadConfigurations) extends NextIterator[Record] with Logging {

  private val client = new AmazonDynamoDBStreamsClient(credentials)
  ...
  override protected def getNext(): Record = {
    ...
    if (toSeqNumberReceived) {
      ...
      if (!internalIterator.hasNext) {
        ...
        lastSeqNumber = nextRecord.getDynamodb.getSequenceNumber
        ...
        if (nextRecord.getDynamodb.getSequenceNumber == range.toSeqNumber) {
...
//    (recordIterator.iterator().asScala, getRecordsResult.getNextShardIterator)
  ...
  private def getKinesisIterator(
    ...
    getShardIteratorRequest.setStreamArn(streamName)
    getShardIteratorRequest.setShardId(shardId)
    getShardIteratorRequest.setShardIteratorType(iteratorType.toString)
    getShardIteratorRequest.setSequenceNumber(sequenceNumber)
