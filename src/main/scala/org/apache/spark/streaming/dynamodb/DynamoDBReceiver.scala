...
import DynamoDBInitialPositions.AtTimestamp
...
/**
  ...
  * @param initialPosition  Instance of [[DynamoDBInitialPosition]]
  *                         In the absence of Kinesis checkpoint info, this is the
  *                         worker's initial starting position in the stream.
  *                         The values are either the beginning of the stream
  *                         per Kinesis' limit of 24 hours
  *                         ([[DynamoDBInitialPositions.TrimHorizon]]) or
  *                         the tip of the stream ([[DynamoDBInitialPositions.Latest]]).
  ...
  */
private[dynamodb] class DynamoDBReceiver[T](
                                           val streamName: String,
                                           endpointUrl: String,
                                           regionName: String,
                                           initialPosition: DynamoDBInitialPosition,
  ...
  /**
    ...
    */
  @volatile private var kinesisCheckpointer: DynamoDBCheckpointer = null
  ...
  override def onStart() {
    ...
    kinesisCheckpointer = new DynamoDBCheckpointer(receiver, checkpointInterval, workerId)
     ...
     */
    val recordProcessorFactory : v2.IRecordProcessorFactory = new v2.IRecordProcessorFactory {
      override def createProcessor: v2.IRecordProcessor =
        new DynamoDBRecordProcessor(receiver, workerId)
    }

    val dynamoDBClient = AmazonDynamoDBClientBuilder.standard
      .withClientConfiguration(kinesisClientLibConfiguration.getDynamoDBClientConfiguration)
      .withCredentials(kinesisClientLibConfiguration.getDynamoDBCredentialsProvider)
      .withRegion(kinesisClientLibConfiguration.getRegionName).build()
    ...
    val cloudWatchClient = AmazonCloudWatchClientBuilder.standard
      ...
      .withCredentials(kinesisClientLibConfiguration.getCloudWatchCredentialsProvider)
      .withRegion(kinesisClientLibConfiguration.getRegionName).build()
    ...
    worker = StreamsWorkerFactory.createDynamoDbStreamsWorker(recordProcessorFactory, kinesisClientLibConfiguration,
  ...
  private[dynamodb] def addRecords(shardId: String, records: java.util.List[Record]): Unit = {
    if (records.size > 0) {
      val dataIterator = records.iterator().asScala.map(messageHandler)
      val metadata = SequenceNumberRange(streamName, shardId,
        records.get(0).getDynamodb.getSequenceNumber(), records.get(records.size() - 1).getDynamodb.getSequenceNumber(),
        records.size())
  ...
  private[dynamodb] def getCurrentLimit: Int = {
  ...
  private[dynamodb] def getLatestSeqNumToCheckpoint(shardId: String): Option[String] = {
