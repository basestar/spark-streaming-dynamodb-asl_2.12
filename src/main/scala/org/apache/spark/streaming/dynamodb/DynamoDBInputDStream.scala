...
package org.apache.spark.streaming.dynamodb
...
import org.apache.spark.streaming.dynamodb.DynamoDBInitialPositions.Latest
...
private[dynamodb] class DynamoDBInputDStream[T: ClassTag](
                                                          _ssc: StreamingContext,
                                                          val streamName: String,
                                                          val endpointUrl: String,
                                                          val regionName: String,
                                                          val initialPosition: DynamoDBInitialPosition,
  ...
  override def createBlockRDD(time: Time, blockInfos: Seq[ReceivedBlockInfo]): RDD[T] = {
    ...
    if (allBlocksHaveRanges) {
      // Create a DynamoDBBackedBlockRDD, even when there are no blocks
      val blockIds = blockInfos.map { _.blockId.asInstanceOf[BlockId] }.toArray
      val seqNumRanges = blockInfos.map {
        _.metadataOption.get.asInstanceOf[SequenceNumberRanges] }.toArray
      val isBlockIdValid = blockInfos.map { _.isBlockIdValid() }.toArray
      logDebug(s"Creating KDynamoDBBackedBlockRDD for $time with ${seqNumRanges.length} " +
        s"seq number ranges: ${seqNumRanges.mkString(", ")} ")

      new DynamoDBBackedBlockRDD(
        context.sc, regionName, endpointUrl, blockIds, seqNumRanges,
        isBlockIdValid = isBlockIdValid,
        messageHandler = messageHandler,
        kinesisCreds = kinesisCreds,
        kinesisReadConfigs = DynamoDBReadConfigurations(ssc))
    } else {
      logWarning("DynamoDB sequence number information was not present with some block metadata," +
  ...
  override def getReceiver(): Receiver[T] = {
    new DynamoDBReceiver(streamName, endpointUrl, regionName, initialPosition,
...
object DynamoDBInputDStream {
  /**
    * Builder for [[DynamoDBInputDStream]] instances.
    *
    * @since 2.2.0
    */
  @InterfaceStability.Evolving
  class Builder {
    ...
    private var initialPosition: Option[DynamoDBInitialPosition] = None
    ...
    /**
      * Sets the StreamingContext that will be used to construct the DynamoDB DStream. This is a
      * required parameter.
      *
      * @param ssc [[StreamingContext]] used to construct DynamoDB DStreams
      * @return Reference to this [[DynamoDBInputDStream.Builder]]
      */
    ...
    /**
      ...
      * required parameter.
      *
      * @param jssc [[JavaStreamingContext]] used to construct DynamoDB DStreams
      * @return Reference to this [[DynamoDBInputDStream.Builder]]
      */
    ...
    /**
      * Sets the name of the DynamoDB stream that the DStream will read from. This is a required
      * parameter.
      *
      * @param streamName Name of DynamoDB stream that the DStream will read from
      * @return Reference to this [[DynamoDBInputDStream.Builder]]
      */
    ...
    /**
      * Sets the AWS DynamoDB endpoint URL. Defaults to "https://kinesis.us-east-1.amazonaws.com" if
    ...
    /**
      * Sets the initial position data is read from in the DynamoDB stream. Defaults to
      * [[DynamoDBInitialPositions.Latest]] if no custom value is specified.
      *
      * @param initialPosition [[DynamoDBInitialPosition]] value specifying where Spark Streaming
      *                        will start reading records in the DynamoDB stream from
      * @return Reference to this [[DynamoDBInputDStream.Builder]]
      */
    def initialPosition(initialPosition: DynamoDBInitialPosition): Builder = {
    ...
    /**
      ...
      * @return Reference to this [[DynamoDBInputDStream.Builder]]
      */
    @deprecated("use initialPosition(initialPosition: DynamoDBInitialPosition)", "2.3.0")
    def initialPositionInStream(initialPosition: InitialPositionInStream): Builder = {
      this.initialPosition = Option(
        DynamoDBInitialPositions.fromKinesisInitialPosition(initialPosition))
      this
    }
    ...
    /**
      * Create a new instance of [[DynamoDBInputDStream]] with configured parameters and the provided
      * message handler.
      *
      * @param handler Function converting [[Record]] instances read by the KCL to DStream type [[T]]
      * @return Instance of [[DynamoDBInputDStream]] constructed with configured parameters
      */
    def buildWithMessageHandler[T: ClassTag](
                                              handler: Record => T): DynamoDBInputDStream[T] = {
      val ssc = getRequiredParam(streamingContext, "streamingContext")
      new DynamoDBInputDStream(
        ssc,
    ...
    /**
      * Create a new instance of [[DynamoDBInputDStream]] with configured parameters and using the
      * default message handler, which returns [[Array[Byte]]].
      *
      * @return Instance of [[DynamoDBInputDStream]] constructed with configured parameters
      */
    def build(): DynamoDBInputDStream[Record] = buildWithMessageHandler(defaultMessageHandler)
  ...
  /**
    * Creates a [[DynamoDBInputDStream.Builder]] for constructing [[DynamoDBInputDStream]] instances.
    *
    * @since 2.2.0
    * @return [[DynamoDBInputDStream.Builder]] instance
    */
  def builder: Builder = new Builder

  private[dynamodb] def defaultMessageHandler(record: Record): Record = {
    record
  }

  private[dynamodb] val DEFAULT_KINESIS_ENDPOINT_URL: String =
    "https://kinesis.us-east-1.amazonaws.com"
  private[dynamodb] val DEFAULT_KINESIS_REGION_NAME: String = "us-east-1"
  private[dynamodb] val DEFAULT_INITIAL_POSITION: DynamoDBInitialPosition = new Latest()
  private[dynamodb] val DEFAULT_STORAGE_LEVEL: StorageLevel = StorageLevel.MEMORY_AND_DISK_2
}
