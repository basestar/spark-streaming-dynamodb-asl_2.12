...
/**
  * Configurations to pass to the [[DynamoDBBackedBlockRDD]].
  *
  ...
  */
private[dynamodb] case class DynamoDBReadConfigurations(
                                                       maxRetries: Int,
                                                       retryWaitTimeMs: Long,
                                                       retryTimeoutMs: Long)

private[dynamodb] object DynamoDBReadConfigurations {
  def apply(): DynamoDBReadConfigurations = {
    DynamoDBReadConfigurations(maxRetries = DEFAULT_MAX_RETRIES,
      retryWaitTimeMs = JavaUtils.timeStringAsMs(DEFAULT_RETRY_WAIT_TIME),
      retryTimeoutMs = DEFAULT_RETRY_TIMEOUT)
  }

  def apply(ssc: StreamingContext): DynamoDBReadConfigurations = {
    DynamoDBReadConfigurations(
