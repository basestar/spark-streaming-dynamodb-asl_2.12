...
/**
  ...
  */
private[dynamodb] class DynamoDBRecordProcessor[T](receiver: DynamoDBReceiver[T], workerId: String)
  extends v2.IRecordProcessor with Logging {
  ...
  override def processRecords(input: ProcessRecordsInput): Unit = {
    ...
    val batch = input.getRecords
    val checkpointer = input.getCheckpointer
    ...
    if (!receiver.isStopped()) {
      try {
        ...
        for (start <- 0 until batch.size by maxRecords) {
          val miniBatch = batch.subList(start, math.min(start + maxRecords, batch.size)).asScala
            .map(v => v.asInstanceOf[RecordAdapter].getInternalObject).asJava
  ...
  override def shutdown(input : ShutdownInput): Unit = {
    ...
    val checkpointer = input.getCheckpointer
    val reason = input.getShutdownReason
...
private[dynamodb] object DynamoDBRecordProcessor extends Logging {
  /**
