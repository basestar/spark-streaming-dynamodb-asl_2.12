...
/**
 * This is a helper class for managing DynamoDB checkpointing.
 *
 ...
 */
private[dynamodb] class DynamoDBCheckpointer(
    receiver: DynamoDBReceiver[_],
   ...
   * checkpoint, e.g. in case of [[com.amazonaws.services.kinesis.clientlibrary.lib.worker.ShutdownReason.ZOMBIE]].
   */
  def removeCheckpointer(shardId: String, checkpointer: IRecordProcessorCheckpointer): Unit = {
    ...
    if (checkpointer != null) {
      try {
        ...
        DynamoDBRecordProcessor.retryRandom(checkpointer.checkpoint(), 4, 100)
      } catch {
        case NonFatal(e) =>
          ...
          // Rethrow the exception to the DynamoDB Worker that is managing this RecordProcessor
          throw e
      }
    }
  }
    ...
    try {
      if (checkpointer != null) {
        receiver.getLatestSeqNumToCheckpoint(shardId).foreach { latestSeqNum =>
          ...
          // DynamoDB sequence numbers are monotonically increasing strings, therefore we can do
          // safely do the string comparison
          if (lastSeqNum == null || latestSeqNum > lastSeqNum) {
            /* Perform the checkpoint */
            DynamoDBRecordProcessor.retryRandom(checkpointer.checkpoint(latestSeqNum), 4, 100)
  ...
  private def startCheckpointerThread(): RecurringTimer = {
    ...
    val threadName = s"DynamoDB Checkpointer - Worker $workerId"
  ...
  def shutdown(): Unit = {
    ...
    logInfo("Successfully shutdown DynamoDB Checkpointer.")
  }
}
